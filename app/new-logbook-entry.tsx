...
import { createLogbookEntry, uploadLogbookEntryPhoto } from '@/src/features/logbook/api';
...
export default function NewLogbookEntryScreen() {
  ...
  // Pré-remplissage OCR : n'écrase jamais une valeur déjà saisie à la main.
  // Sans effet concret pour l'instant (ocrService natif reste un stub, cf. décision produit) —
  // câblé dès maintenant pour ne pas avoir à retoucher cet écran quand le vrai OCR sera branché.
  async function handlePhotoChange(uri: string) {
    setPhotoUri(uri);
    try {
      const result = await ocrService.recognize(uri);
      if (result.km !== undefined && !km) {
